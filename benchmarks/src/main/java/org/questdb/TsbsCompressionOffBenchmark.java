...
public class TsbsCompressionOffBenchmark {

    static final int DISTINCT_SYMBOLS = 4_000;
    static final int FILTERED_SYMBOL = 99;
    static final long LONG_UPPER_BOUND = 5_000;
    static final long ROWS_PER_SYMBOL = 40_000;
    static final long ROWS = ROWS_PER_SYMBOL * DISTINCT_SYMBOLS;
    private static final String DIR = "/tmp/compr-off";
    ...
    public static void main(String[] args) throws RunnerException, IOException, InterruptedException {
        ...
        MemoryCMR imem = Vm.getCMRInstance();
        imem.of(ff, path.of(DIR).concat("test/default/i.d").$(), 0, -1, MemoryTag.MMAP_DEFAULT);
        ...
        long agg = aggregate(FILTERED_SYMBOL, ROWS, imem, l1mem, l2mem);

        System.out.println("Aggregate value: " + agg + ", took: " + (System.nanoTime() - start) / 1_000_000 + "ms");
    }

    private static long aggregate(int filteredValue, long rowCount, MemoryCR imem, MemoryCR l1mem, MemoryCR l2mem) {
        long agg = 0;
        for (long i = 0; i < rowCount; i++) {
            if (imem.getInt(i * Integer.BYTES) == filteredValue) {
    ...
    static void executeDdl(String ddl, CairoConfiguration configuration) {
